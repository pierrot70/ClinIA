...
CLINIQUE_DRILL_SCRIPT="${CLINIQUE_DRILL_SCRIPT:-$ROOT_DIR/scripts/run-staging-clinique-write-drill.sh}"
VERBOSE="${VERBOSE:-0}"
...
run_clinique_drill() {
  ...
  printf '\n=== %s ===\n' "$label"
  VERBOSE="$VERBOSE" "$CLINIQUE_DRILL_SCRIPT"
}
...
[[ -x "$CLINIQUE_DRILL_SCRIPT" ]] || fail "clinique_drill_script_not_executable path=$CLINIQUE_DRILL_SCRIPT"

info "STAGING_CLINIQUE_RESILIENCE_DRILL_STARTED"

run_clinique_drill "cliniques CRUD at 3/3"
...
run_clinique_drill "cliniques CRUD at 2/3 after secondary stop"
start_service "$secondary"
run_clinique_drill "cliniques CRUD after secondary restore"

primary="$(primary_service)" || fail "primary_not_found_before_failover"
stop_service "$primary"
wait_for_new_primary "$primary"
run_clinique_drill "cliniques CRUD after primary failover"
start_service "$primary"
run_clinique_drill "cliniques CRUD after primary restore"
