...
SPECIALIST_DRILL_SCRIPT="${SPECIALIST_DRILL_SCRIPT:-$ROOT_DIR/scripts/run-staging-specialist-write-drill.sh}"
VERBOSE="${VERBOSE:-0}"
...
run_specialist_drill() {
  local label="$1"

  printf '\n=== %s ===\n' "$label"
  VERBOSE="$VERBOSE" "$SPECIALIST_DRILL_SCRIPT"
}
...
[[ -x "$SPECIALIST_DRILL_SCRIPT" ]] || fail "specialist_drill_script_not_executable path=$SPECIALIST_DRILL_SCRIPT"

info "STAGING_SPECIALIST_RESILIENCE_DRILL_STARTED"

run_specialist_drill "specialists CRUD at 3/3"
...
run_specialist_drill "specialists CRUD at 2/3 after secondary stop"
start_service "$secondary"
run_specialist_drill "specialists CRUD after secondary restore"

primary="$(primary_service)" || fail "primary_not_found_before_failover"
stop_service "$primary"
wait_for_new_primary "$primary"
run_specialist_drill "specialists CRUD after primary failover"
start_service "$primary"
run_specialist_drill "specialists CRUD after primary restore"
