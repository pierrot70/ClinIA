...
DIAGNOSIS_DRILL_SCRIPT="${DIAGNOSIS_DRILL_SCRIPT:-$ROOT_DIR/scripts/run-staging-diagnosis-result-write-drill.sh}"
VERBOSE="${VERBOSE:-0}"
...
run_diagnosis_drill() {
  ...
  printf '\n=== %s ===\n' "$label"
  VERBOSE="$VERBOSE" "$DIAGNOSIS_DRILL_SCRIPT"
}
...
[[ -x "$DIAGNOSIS_DRILL_SCRIPT" ]] || fail "diagnosis_drill_script_not_executable path=$DIAGNOSIS_DRILL_SCRIPT"

info "STAGING_DIAGNOSIS_RESULT_RESILIENCE_DRILL_STARTED"

run_diagnosis_drill "diagnosis results CRUD at 3/3"
...
run_diagnosis_drill "diagnosis results CRUD at 2/3 after secondary stop"
start_service "$secondary"
run_diagnosis_drill "diagnosis results CRUD after secondary restore"

primary="$(primary_service)" || fail "primary_not_found_before_failover"
stop_service "$primary"
wait_for_new_primary "$primary"
run_diagnosis_drill "diagnosis results CRUD after primary failover"
start_service "$primary"
run_diagnosis_drill "diagnosis results CRUD after primary restore"
