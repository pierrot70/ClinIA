...
PATIENT_DRILL_SCRIPT="${PATIENT_DRILL_SCRIPT:-$ROOT_DIR/scripts/run-staging-patient-write-drill.sh}"
VERBOSE="${VERBOSE:-0}"
...
run_patient_drill() {
  ...
  printf '\n=== %s ===\n' "$label"
  VERBOSE="$VERBOSE" "$PATIENT_DRILL_SCRIPT"
}
...
[[ -x "$PATIENT_DRILL_SCRIPT" ]] || fail "patient_drill_script_not_executable path=$PATIENT_DRILL_SCRIPT"

info "STAGING_PATIENT_RESILIENCE_DRILL_STARTED"

run_patient_drill "patients CRUD at 3/3"
...
run_patient_drill "patients CRUD at 2/3 after secondary stop"
start_service "$secondary"
run_patient_drill "patients CRUD after secondary restore"

primary="$(primary_service)" || fail "primary_not_found_before_failover"
stop_service "$primary"
wait_for_new_primary "$primary"
run_patient_drill "patients CRUD after primary failover"
start_service "$primary"
run_patient_drill "patients CRUD after primary restore"
