...
COMMENT_DRILL_SCRIPT="${COMMENT_DRILL_SCRIPT:-$ROOT_DIR/scripts/run-staging-clinician-comment-write-drill.sh}"
VERBOSE="${VERBOSE:-0}"
...
run_comment_drill() {
  ...
  printf '\n=== %s ===\n' "$label"
  VERBOSE="$VERBOSE" "$COMMENT_DRILL_SCRIPT"
}
...
[[ -x "$COMMENT_DRILL_SCRIPT" ]] || fail "comment_drill_script_not_executable path=$COMMENT_DRILL_SCRIPT"

info "STAGING_CLINICIAN_COMMENT_RESILIENCE_DRILL_STARTED"

run_comment_drill "clinician comments CRUD at 3/3"
...
run_comment_drill "clinician comments CRUD at 2/3 after secondary stop"
start_service "$secondary"
run_comment_drill "clinician comments CRUD after secondary restore"

primary="$(primary_service)" || fail "primary_not_found_before_failover"
stop_service "$primary"
wait_for_new_primary "$primary"
run_comment_drill "clinician comments CRUD after primary failover"
start_service "$primary"
run_comment_drill "clinician comments CRUD after primary restore"
