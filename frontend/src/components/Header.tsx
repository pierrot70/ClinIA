...
const HEADER_TRANSLATION_KEYS: Record<string, string> = {
    ...
    "AI real": "header.aiMode.real",
    "AI mock": "header.aiMode.mock",
    "Forcer IA réelle": "header.aiMode.forceRealTitle",
    "Utiliser le mode mock": "header.aiMode.mockTitle",
    Accueil: "header.nav.home",
    Plus: "header.nav.more",
    Collaboration: "header.nav.collaboration",
    Outils: "header.nav.tools",
...
const Header: React.FC = () => {
    ...
    const isDev = !isProd;
    // The public header must not disclose clinical, administrative, or
    // development navigation.  Routes remain protected independently.
    const showFullHeaderNav = isAuthenticated;
        ...
        <header className="sticky top-0 z-50 border-b border-gray-200 bg-white lg:fixed lg:inset-x-0 lg:h-16">
                ...
                <nav className="mt-3 hidden items-center gap-4 text-sm lg:fixed lg:inset-y-0 lg:left-0 lg:mt-0 lg:flex lg:w-64 lg:flex-col lg:items-stretch lg:gap-1 lg:overflow-y-auto lg:border-r lg:border-slate-200 lg:bg-slate-50 lg:px-3 lg:py-5 lg:shadow-sm">
                    ...
                    {showFullHeaderNav && (
                        <>
                            ...
                            <Link to="/my-write-receipts" className={linkClass("/my-write-receipts")}>
                                <HeaderLabel text={headerLabels.nav.myWriteReceipts} />
                            </Link>

                            {user?.role === "MEDECIN" && (
                                <Link to="/consultations" className={linkClass("/consultations")}>
                                    {consultationLabels(locale).title}
                                </Link>
                            )}
                            {user?.role === "MEDECIN" && (
                                    ...
                                    {[
                                        ...
                                        ["/cliniques", "/specialists"].includes(item.path) ? (
                                            <span
                                                key={item.path}
                                                aria-disabled="true"
                                                className="block cursor-not-allowed px-4 py-2 text-sm text-gray-400"
                                            >
                    ...
                    {showAdminHeaderNav && (
                        <Link
                            to="/admin/write-operation-audits"
                            className={
                                "hover:text-blue-600 transition-colors " +
                                (location.pathname === "/admin/write-operation-audits"
                                    ? "text-blue-600 font-medium"
                                    : "text-gray-600")
                            }
                        >
                            <HeaderLabel text={headerLabels.nav.dbAudits} />
                        </Link>
                    )}
                ...
                {isMobileMenuOpen && (
                    <div className="fixed inset-y-0 left-0 z-[60] w-[86vw] max-w-sm space-y-3 overflow-y-auto border-r border-slate-200 bg-white p-4 text-gray-900 shadow-2xl lg:hidden">
                        ...
                        <div className="space-y-1 border-t border-gray-100 pt-2">
                            {isPhysicianMobileNav && (
                                <div className="px-2 pb-1 text-xs font-semibold uppercase tracking-wide text-slate-500">
                                    <HeaderLabel text={headerLabels.nav.collaboration} />
                                </div>
                            )}
                            ...
                            {user?.role === "MEDECIN" && <Link to="/consultations" onClick={() => setIsMobileMenuOpen(false)} className="block rounded px-2 py-2 text-sm text-gray-700 hover:bg-gray-50">{consultationLabels(locale).title}</Link>}
                            ...
                            {user?.role === "MEDECIN" ? (
                                <span aria-disabled="true" className="block cursor-not-allowed rounded px-2 py-2 text-sm text-gray-400"><HeaderLabel text={headerLabels.nav.cliniques} /></span>
                            ) : (
                            ...
                            {user?.role === "MEDECIN" ? (
                                <span aria-disabled="true" className="block cursor-not-allowed rounded px-2 py-2 text-sm text-gray-400"><HeaderLabel text={headerLabels.nav.specialists} /></span>
                            ) : (
                            ...
                            {isPhysicianMobileNav && (
                                <div className="mt-3 border-t border-gray-100 px-2 pb-1 pt-3 text-xs font-semibold uppercase tracking-wide text-slate-500">
                                    <HeaderLabel text={headerLabels.nav.tools} />
                                </div>
                            )}
                        ...
                        <div className="space-y-1 border-t border-gray-100 pt-2">
                            ...
                            {showAdminHeaderNav && (
                                <Link to="/admin/write-operation-audits" onClick={() => setIsMobileMenuOpen(false)} className="block rounded px-2 py-2 text-sm text-gray-700 hover:bg-gray-50"><HeaderLabel text={headerLabels.nav.dbAudits} /></Link>
                            )}
            ...
            {false && hasMobileBottomNav && (
                <nav className="order-3 z-50 grid shrink-0 grid-cols-4 border-t border-slate-200 bg-white px-2 pb-[env(safe-area-inset-bottom)] pt-2 shadow-[0_-6px_20px_rgba(15,23,42,0.12)] lg:hidden">
                    ...
                    <Link to="/clinical" onClick={() => setIsMobileMenuOpen(false)} className={`flex min-h-14 flex-col items-center justify-center rounded-lg text-center text-xs ${location.pathname === "/clinical" ? "bg-blue-50 font-semibold text-blue-700" : "text-slate-600"}`}>
                        <span className="text-base" aria-hidden="true">✦</span>
                        <HeaderLabel text={headerLabels.nav.clinicalAnalysis} />
                    </Link>
                    {isPhysicianMobileNav ? (
